...
describe('URI', function() {
  ...
  it('parses uri strings correctly (test vector)', function() {
    ...
    uri = URI.parse('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    uri.address.should.equal('1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    expect(uri.amount).to.be.undefined();
    expect(uri.otherParam).to.be.undefined();

    uri = URI.parse('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=123.22');
    uri.address.should.equal('1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  ...
  it('URIs can be validated statically (test vector)', function() {
    URI.isValid('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj').should.equal(true);
    URI.isValid('anon:mkYY5NRvikVBY1EPtaq9fAFgquesdjqECw').should.equal(true);

    URI.isValid('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2')
                .should.equal(true);
    URI.isValid('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param')
                .should.equal(true);
    URI.isValid('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&req-other=param',
                ['req-other']).should.equal(true);
    URI.isValid('anon:mmrqEBJxUCf42vdb3oozZtyz5mKr3Vb2Em?amount=0.1&' +
    ...
    URI.isValid('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfk?amount=bad').should.equal(false);
    URI.isValid('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfk?amount=1.2&req-other=param')
                .should.equal(false);
  ...
  it('do not need new keyword', function() {
    var uri = URI('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  ...
  describe('instantiation from bitcoin uri', function() {
    ...
    it('parses address', function() {
      uri = new URI('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.livenet);
    });

    it('parses amount', function() {
      uri = URI.fromString('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=123.22');
    ...
    it('parses a testnet address', function() {
      uri = new URI('anon:mkYY5NRvikVBY1EPtaq9fAFgquesdjqECw');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.testnet);
    });

    it('stores unknown parameters as "extras"', function() {
      uri = new URI('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param');
      uri.address.should.be.instanceof(bitcore.Address);
    ...
    it('throws error when a required feature is not supported', function() {
      (function() {
        return new URI('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param&req-required=param');
      }).should.throw(Error);
    });
  ...
  it('should support double slash scheme', function() {
    var uri = new URI('anon://1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    uri.address.toString().should.equal('1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  });

  it('should input/output String', function() {
    var str = 'anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?' +
  ...
  it('should support numeric amounts', function() {
    var uri = new URI('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=12.10001');
  ...
  it('should generate a valid URI', function() {
    new URI({
      ...
      'anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj'
    );
    ...
    new URI({
      ...
      'anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.10001&message=Hello%20World&something=else'
    );

  });

  it('should be case insensitive to protocol', function() {
    var uri1 = new URI('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    var uri2 = new URI('anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');

    uri1.address.toString().should.equal(uri2.address.toString());
  });

  it('writes correctly the "r" parameter on string serialization', function() {
    var originalString = 'anon:mmrqEBJxUCf42vdb3oozZtyz5mKr3Vb2Em?amount=0.1&' +
  ...
  it('displays nicely on the console (#inspect)', function() {
    var uri = 'anon:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj';
