...
describe('URI', function() {
  ...
  it('parses uri strings correctly (test vector)', function() {
    ...
    uri = URI.parse('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');
  ...
  it('URIs can be validated statically (test vector)', function() {
    URI.isValid('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ').should.equal(true);
    URI.isValid('anon:tACY3B38bjSux2bzTvXxbSG4CaomNFPeykZ').should.equal(true);

    URI.isValid('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?amount=1.2')
                .should.equal(true);
    URI.isValid('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?amount=1.2&other=param')
                .should.equal(true);
  ...
  it('do not need new keyword', function() {
    var uri = URI('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');
  ...
  describe('instantiation from bitcoin uri', function() {
    ...
    it('parses address', function() {
      uri = new URI('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.livenet);
    });

    it('parses amount', function() {
      uri = URI.fromString('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?amount=123.22');
      uri.address.toString().should.equal('AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');
      uri.amount.should.equal(12322000000);
      expect(uri.otherParam).to.be.undefined();
    });

    it('parses a testnet address', function() {
      uri = new URI('anon:tACY3B38bjSux2bzTvXxbSG4CaomNFPeykZ');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.testnet);
    });

    it('stores unknown parameters as "extras"', function() {
      uri = new URI('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?amount=1.2&other=param');
      uri.address.should.be.instanceof(bitcore.Address);
    ...
    it('throws error when a required feature is not supported', function() {
      (function() {
        return new URI('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?amount=1.2&other=param&req-required=param');
      }).should.throw(Error);
    });
  ...
  it('should create instance from object', function() {
    ...
    uri = new URI({
      address: 'AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ'
    });
    uri.address.should.be.instanceof(bitcore.Address);
    uri.network.should.equal(Networks.livenet);

    uri = new URI({
      address: 'tACY3B38bjSux2bzTvXxbSG4CaomNFPeykZ'
    });
    uri.address.should.be.instanceof(bitcore.Address);
    uri.network.should.equal(Networks.testnet);
  ...
  it('should support double slash scheme', function() {
    var uri = new URI('anon://AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');
    uri.address.toString().should.equal('AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');
  });

  it('should input/output String', function() {
    var str = 'anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?' +
  ...
  it('should support numeric amounts', function() {
    var uri = new URI('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?amount=12.10001');
  ...
  it('should generate a valid URI', function() {
    new URI({
      ...
      'anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ'
    );
    ...
    new URI({
      ...
      'anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ?amount=1.10001&message=Hello%20World&something=else'
    );

  });

  it('should be case insensitive to protocol', function() {
    var uri1 = new URI('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');
    var uri2 = new URI('anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ');

    uri1.address.toString().should.equal(uri2.address.toString());
  });

  it('writes correctly the "r" parameter on string serialization', function() {
    var originalString = 'anon:tAT953y4AKxaijucuhQmvXMDVSRcqthDFhG?amount=0.1&' +
  ...
  it('displays nicely on the console (#inspect)', function() {
    var uri = 'anon:AniknENWKqvMSAXZYEGPdFtRrKabRKqgEVJ';
